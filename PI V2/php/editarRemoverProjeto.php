...
if (isset($_POST['edita'])) {
    ...
    $stmt = $bd -> prepare(
        "UPDATE projeto_PP SET nomeProjeto = '$nomeProjeto', dataPostagem = '$data', ativo = '$ativo', imagens = '$arquivoEnviado', descricao = '$desc'
    ...
    if (isset($_POST['deleta'])) {
        $stmt = $bd -> prepare("DELETE FROM projeto_PP WHERE IdProjeto = {$_GET['id']}");
        if ($stmt -> execute()) {
            header('location: ../paginas/meusPortifolios.php');
        }
    }
