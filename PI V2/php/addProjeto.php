...
$stmt = $bd -> prepare("SELECT IdUsuario FROM usuario_UU WHERE nomeUsuario = '$nomeUsuario'");
$stmt -> execute();
...
$stmt = $bd -> prepare(
    "INSERT INTO projeto_PP (IdProjeto, IdUsuario, IdCategoria, nomeProjeto, imagens, dataPostagem, descricao, ativo) 
...
$stmt = $bd -> query("SELECT (MAX(IdProjeto)) AS ID FROM projeto_PP WHERE nomeProjeto = '$nomeProjeto'");
$stmt -> execute();
$val = $stmt -> fetch(PDO::FETCH_ASSOC);
